...
import FaturaScanner from '@/components/FaturaScanner'
...
type Det = {
  ...
  statusi:string; shenime:string; progres_pct:number; foto_url:string;
...
export default function Detyrimet() {
  ...
  // OCR Result Handler
  const handleOCR = (data: Record<string,string|null>, imageUrl: string) => {
    setForm(prev => ({
      ...prev,
      pala: data.pala || prev.pala,
      pershkrimi: data.pershkrimi || prev.pershkrimi,
      referenca: data.referenca || prev.referenca,
      shuma_totale: data.shuma_totale || prev.shuma_totale,
      data_fillimit: data.data_fillimit || prev.data_fillimit,
      data_skadimit: data.data_skadimit || prev.data_skadimit,
      shenime: data.shenime || prev.shenime,
      foto_url: imageUrl || prev.foto_url,
    }))
    setOcrDone(true)
    showT('✓ Të dhënat u lexuan nga foto!')
  }

  const saveDetyrimi = async () => {
    if (!form.pala||!form.shuma_totale||!form.pershkrimi) { showT('Plotëso: Pala, Shuma, Përshkrimi'); return }
    setSaving(true)
    ...
    setSaving(false); setShowAdd(false); setOcrDone(false)
    setForm({...EMPTY_FORM})
  ...
  if (selected) return (
    <div style={{padding:'20px 16px',minHeight:'100vh'}}>
      ...
      <div className="au" style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'20px'}}>
        <button onClick={()=>setSelected(null)} style={{width:'36px',height:'36px',background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.1)',borderRadius:'10px',color:'#fff',fontSize:'18px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
        <div>
      ...
      {/* Foto faturës nëse ekziston */}
      {selected.foto_url && (
        <div className="au" style={{marginBottom:'14px',animationDelay:'.02s'}}>
          <img src={selected.foto_url} alt="Fatura" style={{width:'100%',borderRadius:'14px',maxHeight:'180px',objectFit:'cover',border:'1px solid rgba(201,168,76,0.2)',cursor:'pointer'}}
            onClick={()=>window.open(selected.foto_url,'_blank')} />
          <p style={{fontSize:'10px',color:'#555',textAlign:'center',margin:'6px 0 0'}}>📎 Klikoni për të parë dokumentin i plotë</p>
        </div>
      )}
      ...
      <div className="au" style={{background:'linear-gradient(135deg,#111118,#1a1508)',border:'1px solid rgba(201,168,76,0.25)',borderRadius:'20px',padding:'20px',marginBottom:'14px',animationDelay:'.05s'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'12px',marginBottom:'16px'}}>
          {[['Totale','#fff',selected.shuma_totale],['Paguar','#4ade80',selected.shuma_paguar],['Mbetur','#f87171',selected.shuma_mbetur]].map(([lbl2,col,val],i)=>(
            <div key={i}>
              <p style={{fontSize:'9px',color:'#55555588',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>{lbl2 as string}</p>
              <p style={{fontSize:'16px',fontWeight:'800',color:col as string,margin:0,fontVariantNumeric:'tabular-nums'}}>{fmt(Number(val))}</p>
              <p style={{fontSize:'9px',color:'#444',margin:'1px 0 0'}}>LEK</p>
            </div>
          ))}
        </div>
        ...
        <div style={{display:'flex',justifyContent:'space-between'}}>
          <span style={{fontSize:'10px',color:'#555'}}>{fmt(Number(selected.shuma_paguar))} paguar</span>
      ...
      <div className="au" style={{background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.07)',borderRadius:'16px',padding:'16px',marginBottom:'14px',animationDelay:'.1s'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
          {[
            ['Interesa',`${selected.norma_interesit||0}%`,'#f59e0b'],
            ['Kesti','LEK',fmt(Number(selected.kesti_mujor)),'#67e8f9'],
            ...
            selected.data_mbarimit?['Mbarimi',fmtDate(selected.data_mbarimit),'#888']:['Skadenca',selected.data_skadimit?fmtDate(selected.data_skadimit):'—','#f59e0b'],
            selected.referenca?['Referenca',selected.referenca,'#888']:['Pagesa',`${selected.pagesa_totale||0} total`,'#888'],
          ].map(([l,v,c],i)=>(
            <div key={i} style={{background:'rgba(255,255,255,.03)',borderRadius:'10px',padding:'10px'}}>
              <p style={{fontSize:'9px',color:'#444',textTransform:'uppercase',letterSpacing:'.5px',margin:'0 0 3px'}}>{l as string}</p>
              <p style={{fontSize:'13px',fontWeight:'700',color:c as string,margin:0,fontVariantNumeric:'tabular-nums',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{v as string}</p>
            </div>
          ))}
        </div>
        {selected.pershkrimi && <div style={{marginTop:'10px',paddingTop:'10px',borderTop:'1px solid rgba(255,255,255,.06)'}}>
          <p style={{fontSize:'9px',color:'#444',margin:'0 0 3px',textTransform:'uppercase',letterSpacing:'.5px'}}>Përshkrimi</p>
          <p style={{fontSize:'12px',color:'#ccc',margin:0}}>{selected.pershkrimi}</p>
        </div>}
        {selected.shenime && <p style={{fontSize:'11px',color:'#444',margin:'8px 0 0',fontStyle:'italic'}}>{selected.shenime}</p>}
      </div>

      {/* ADD PAYMENT */}
      <button onClick={()=>setShowPay(!showPay)} style={{width:'100%',padding:'13px',background:showPay?'rgba(201,168,76,.1)':'linear-gradient(135deg,#c9a84c,#f0c060)',border:showPay?'1px solid rgba(201,168,76,.3)':'none',borderRadius:'14px',color:showPay?'#c9a84c':'#0a0a0f',fontSize:'14px',fontWeight:'800',cursor:'pointer',marginBottom:'14px',fontFamily:'inherit'}}>
      ...
      {showPay && (
        <div className="au" style={{background:'rgba(201,168,76,.05)',border:'1px solid rgba(201,168,76,.2)',borderRadius:'16px',padding:'16px',marginBottom:'14px'}}>
          <p style={{fontSize:'11px',fontWeight:'700',color:'#c9a84c',textTransform:'uppercase',letterSpacing:'1px',margin:'0 0 12px'}}>Pagese e Re</p>
          <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              <div><p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Data</p>
                <input type="date" value={payForm.data_pageses} onChange={e=>setPayForm(p=>({...p,data_pageses:e.target.value}))}/></div>
              <div><p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Shuma *</p>
                <input type="number" placeholder={selected.kesti_mujor?String(selected.kesti_mujor):'0'} value={payForm.shuma} onChange={e=>setPayForm(p=>({...p,shuma:e.target.value}))}/></div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              <div><p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Principal (LEK)</p>
                <input type="number" placeholder="0" value={payForm.shuma_principal} onChange={e=>setPayForm(p=>({...p,shuma_principal:e.target.value}))}/></div>
              <div><p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Interesa (LEK)</p>
                <input type="number" placeholder="0" value={payForm.shuma_interesa} onChange={e=>setPayForm(p=>({...p,shuma_interesa:e.target.value}))}/></div>
            </div>
            <div><p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Statusi</p>
            ...
            <div><p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Referenca</p>
              <input type="text" placeholder="Nr. faturë opsional" value={payForm.referenca} onChange={e=>setPayForm(p=>({...p,referenca:e.target.value}))}/></div>
            <button onClick={savePagese} disabled={saving} style={{padding:'12px',background:'linear-gradient(135deg,#c9a84c,#f0c060)',border:'none',borderRadius:'12px',color:'#0a0a0f',fontSize:'13px',fontWeight:'800',cursor:'pointer',marginTop:'4px'}}>
      ...
      <div className="au" style={{background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.07)',borderRadius:'16px',padding:'16px',animationDelay:'.15s'}}>
        <p style={{fontSize:'11px',fontWeight:'700',color:'#fff',margin:'0 0 14px',textTransform:'uppercase',letterSpacing:'.8px'}}>Historia e Pagesave ({pagesat.length})</p>
        {pagesat.length===0 ? (
          <div style={{textAlign:'center',padding:'20px',color:'#333'}}>
            <p style={{fontSize:'20px',margin:'0 0 6px'}}>📋</p>
            <p style={{fontSize:'11px',margin:0}}>Nuk ka pagesa akoma</p>
          </div>
        ) : pagesat.map((p,i)=>(
          <div key={p.id} style={{display:'flex',alignItems:'center',gap:'10px',padding:'10px 0',borderBottom:i<pagesat.length-1?'1px solid rgba(255,255,255,.05)':'none'}}>
            <div style={{width:'36px',height:'36px',borderRadius:'10px',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'16px',
              background:p.statusi==='paguar'?'rgba(74,222,128,.1)':p.statusi==='vonuar'?'rgba(248,113,113,.1)':'rgba(245,158,11,.1)'}}>
              {p.statusi==='paguar'?'✓':p.statusi==='vonuar'?'!':'○'}
            </div>
            <div style={{flex:1,minWidth:0}}>
              <p style={{fontSize:'13px',fontWeight:'700',color:'#fff',margin:'0 0 2px',fontVariantNumeric:'tabular-nums'}}>{fmt(Number(p.shuma))} LEK</p>
            ...
            <span style={{fontSize:'10px',fontWeight:'600',padding:'3px 10px',borderRadius:'10px',flexShrink:0,
              background:p.statusi==='paguar'?'rgba(74,222,128,.15)':p.statusi==='vonuar'?'rgba(248,113,113,.15)':'rgba(245,158,11,.15)',
              color:p.statusi==='paguar'?'#4ade80':p.statusi==='vonuar'?'#f87171':'#f59e0b'}}>
              {ST_LABEL[p.statusi]||p.statusi}
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{padding:'20px 16px',display:'flex',flexDirection:'column',gap:'14px',minHeight:'100vh'}}>
      ...
      <div className="au" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',animationDelay:'.06s'}}>
        <div style={{background:'rgba(248,113,113,.08)',border:'1px solid rgba(248,113,113,.2)',borderRadius:'14px',padding:'12px 8px',textAlign:'center'}}>
          <p style={{fontSize:'9px',color:'#f8717188',textTransform:'uppercase',letterSpacing:'.6px',margin:'0 0 5px'}}>Borxh Mbetur</p>
          <p style={{fontSize:'15px',fontWeight:'800',color:'#f87171',margin:0,fontVariantNumeric:'tabular-nums'}}>{fmt(totals.mbetur)}</p>
          <p style={{fontSize:'9px',color:'#555',margin:'2px 0 0'}}>LEK</p>
        </div>
        <div style={{background:'rgba(74,222,128,.08)',border:'1px solid rgba(74,222,128,.2)',borderRadius:'14px',padding:'12px 8px',textAlign:'center'}}>
          <p style={{fontSize:'9px',color:'#4ade8088',textTransform:'uppercase',letterSpacing:'.6px',margin:'0 0 5px'}}>Paguar</p>
          <p style={{fontSize:'15px',fontWeight:'800',color:'#4ade80',margin:0,fontVariantNumeric:'tabular-nums'}}>{fmt(totals.paguar)}</p>
          <p style={{fontSize:'9px',color:'#555',margin:'2px 0 0'}}>LEK</p>
        </div>
        <div style={{background:totals.vonuar>0?'rgba(248,113,113,.08)':'rgba(255,255,255,.03)',border:`1px solid ${totals.vonuar>0?'rgba(248,113,113,.2)':'rgba(255,255,255,.07)'}`,borderRadius:'14px',padding:'12px 8px',textAlign:'center'}}>
          <p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.6px',margin:'0 0 5px'}}>Vonuar</p>
          <p style={{fontSize:'15px',fontWeight:'800',color:totals.vonuar>0?'#f87171':'#444',margin:0}}>{totals.vonuar}</p>
      ...
      {/* ADD FORM */}
      {showAdd && (
        <div className="au" style={{background:'rgba(201,168,76,.04)',border:'1px solid rgba(201,168,76,.2)',borderRadius:'16px',padding:'16px',animationDelay:'0s'}}>
          <p style={{fontSize:'11px',fontWeight:'700',color:'#c9a84c',textTransform:'uppercase',letterSpacing:'1px',margin:'0 0 14px'}}>Detyrimi i Ri</p>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'5px',marginBottom:'14px'}}>
            {LLOJET.map(l=>(
              <button key={l.id} onClick={()=>setForm(p=>({...p,lloji:l.id}))}
                style={{padding:'8px 3px',borderRadius:'10px',border:`1px solid ${form.lloji===l.id?l.color:'rgba(255,255,255,.08)'}`,
                ...
                <span style={{fontSize:'7px',color:form.lloji===l.id?l.color:'#444',textAlign:'center',lineHeight:1.2}}>{l.label.split(' ').slice(0,1).join('')}</span>
              </button>
            ))}
          </div>

          {/* OCR SCANNER - vetëm për fatura */}
          {(form.lloji==='fature_e_papaguar'||form.lloji==='borxh_i_trete') && (
            <div style={{marginBottom:'14px'}}>
              <p style={{fontSize:'9px',color:'#c9a84c',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 8px',display:'flex',alignItems:'center',gap:'6px'}}>
                <span>📸</span> Skano Faturën me AI (opsional)
              </p>
              <FaturaScanner
                onResult={handleOCR}
                supabaseUrl={SB_URL}
                supabaseKey={SB_KEY}
              />
              {ocrDone && <p style={{fontSize:'10px',color:'#4ade80',margin:'0 0 10px',textAlign:'center'}}>✓ Fushat u plotësuan automatikisht — kontrolloni poshtë</p>}
            </div>
          )}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'9px'}}>
            ...
            <div>
              ...
              <input type="text" placeholder="p.sh. Faturë Nr. 1234 - Punime ndërtimi" value={form.pershkrimi} onChange={e=>setForm(p=>({...p,pershkrimi:e.target.value}))}/>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              <div>
                <p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Shuma (LEK) *</p>
                <input type="number" placeholder="0" value={form.shuma_totale} onChange={e=>setForm(p=>({...p,shuma_totale:e.target.value}))}/>
              </div>
              <div>
                <p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>Nr. Faturë / Referenca</p>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              ...
              <div>
                <p style={{fontSize:'9px',color:'#888',textTransform:'uppercase',letterSpacing:'.8px',margin:'0 0 4px'}}>{form.lloji==='fature_e_papaguar'||form.lloji==='borxh_i_trete'?'Skadenca':'Mbarimi'}</p>
                <input type="date" value={form.lloji==='fature_e_papaguar'||form.lloji==='borxh_i_trete'?form.data_skadimit:form.data_mbarimit}
                  onChange={e=>setForm(p=>form.lloji==='fature_e_papaguar'||form.lloji==='borxh_i_trete'?{...p,data_skadimit:e.target.value}:{...p,data_mbarimit:e.target.value})}/>
              </div>
            </div>
            <div>
      ...
      <div className="au" style={{display:'flex',gap:'6px',overflowX:'auto',animationDelay:'.1s',scrollbarWidth:'none' as const}}>
        <button onClick={()=>setFilter('all')} style={{flexShrink:0,padding:'6px 14px',borderRadius:'20px',border:'1px solid',fontSize:'11px',fontWeight:'600',cursor:'pointer',fontFamily:'inherit',
          background:filter==='all'?'#c9a84c':'transparent',color:filter==='all'?'#0a0a0f':'#666',borderColor:filter==='all'?'#c9a84c':'rgba(255,255,255,.1)'}}>
          Të gjitha {items.length}
        </button>
        {LLOJET.map(l=>(
          <button key={l.id} onClick={()=>setFilter(l.id)} style={{flexShrink:0,padding:'6px 12px',borderRadius:'20px',border:'1px solid',fontSize:'10px',fontWeight:'600',cursor:'pointer',fontFamily:'inherit',
            background:filter===l.id?`${l.color}18`:'transparent',color:filter===l.id?l.color:'#555',borderColor:filter===l.id?`${l.color}50`:'rgba(255,255,255,.07)'}}>
      ...
      {loading ? (
        ...
        <div className="au" style={{background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.07)',borderRadius:'16px',padding:'40px',textAlign:'center'}}>
          ...
          <p style={{fontSize:'12px',color:'#555',margin:'0 0 16px'}}>Fotografo faturën ose shto manualisht</p>
          ...
          {filtered.map((d,i)=>{
            ...
            return (
              <div key={d.id} className="au" onClick={()=>openDetail(d)}
                style={{background:'rgba(255,255,255,.03)',border:`1px solid ${d.statusi==='vonuar'?'rgba(248,113,113,.3)':'rgba(255,255,255,.07)'}`,borderRadius:'16px',padding:'16px',cursor:'pointer',animationDelay:`${.12+i*.05}s`,position:'relative' as const}}>
                {d.foto_url && <div style={{position:'absolute',top:'12px',right:'12px',fontSize:'14px',opacity:.5}}>📎</div>}
                <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'10px'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{fontSize:'14px',fontWeight:'700',color:'#fff',margin:'0 0 3px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.pala}</p>
                  ...
                  <div style={{textAlign:'right',flexShrink:0}}>
                    <p style={{fontSize:'16px',fontWeight:'800',color:'#f87171',margin:'0 0 1px',fontVariantNumeric:'tabular-nums'}}>{fmt(Number(d.shuma_mbetur))}</p>
                    <p style={{fontSize:'9px',color:'#444',margin:0}}>LEK mbetur</p>
                  </div>
                </div>
                ...
                <div style={{display:'flex',justifyContent:'space-between'}}>
                  <span style={{fontSize:'10px',color:'#444'}}>{fmt(Number(d.shuma_paguar))} / {fmt(Number(d.shuma_totale))} LEK</span>
